...
An advanced blockchain explorer for the Scp network, built on Node.js + SQL.
...
- Simple setup: Only a synced Scp client and Node.js are required. No need to install a database engine, as a self-contained SQLite database is used by
  default
...
- It follows consensus rules not included on the raw blockchain, as indicating when a contract failed or succeded or the dividend paid to ScpFund holders during a contract formation
...
- Search transactions in bulk to get an overview of a whole Scp wallet. Up to 100.000 addresses can be queried this way
...
- The most stable and reliable version of Navigator ever. The Scp `explorer` module is not necessary anymore, what results in more accurate data, no downtime due to critical bugs on the module and setups easier to maintain (by orders of magnitude)
...
- A Scp client running and fully synced to the blockchain
...
### Using or not the `explorer` module of Scp

Navigator can work without enabling the `explorer` module of Scp. However, due to some information missing from the `/consensus` API endpoint of Scp, it is not possible to know the output ID of miner payouts and ScpFund claimed coins. These amounts will show up correctly in Navigator, but when these amounts are spent, the addresses will not show the deducted coins. In other words, if you don't enable the explorer module, every transaction will show correctly, but the balances of SCP of mining pools and SPF owners will not be accurate. Only these two kinds of wallets (a marginally small percentage of the users) are impacted, the rest of wallets will be accurate.

If you optionally enable the `explorer` Scp module (take in mind it will take on its own ~3GB on your Scp folder) and set to `true` the variable `explorerAvailable` on the config file of Navigator, `/explorer` calls will be used to complete this missing information and show 100% accurate data for these special wallets. Even if the `explorer` module of Scp is deprecated and unstable, Navigator will keep working perfectly, even after the frequent crtical errors of this module. In other words, even if this Scp module is unstable, Navigator will not be impacted and you will not need to perform any additional maintenance.

Once the Scp developers fix the issue [#3791](https://gitlab.com/NebulousLabs/Sia/-/issues/3791), a future update of Navigator will drop this last point where the `explorer` module can be helpful.
...
## Compatibility with other blockchains and Scp forks

Navigator is a blockchain explorer fully tailored to the Scp blockchain and I will gladly assist any developer aiming to deploy a mirror of it or modify it as long as it remains in the scope of the Scp blockchain. The code will **not** work out of the box for other forks of Scp or blockchains, present or future, and will require extensive modifications due to the many particularities and specificities of each blockchain. Navigator is open source and you are free to try to adapt it to other blockchain, but it will be your full responsability to ensure its accuracy and I will not assist you in this task.
